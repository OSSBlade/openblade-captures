...
$outputDirectory = 'C:\tmp\openblade-balanced-validation-v1-20260716'
$stateFile = 'C:\tmp\openblade-balanced-validation-v1-state.txt'
...
try {
    ...
    $process = Start-Process -FilePath $capture `
        ...
        -PassThru `
        -WindowStyle Hidden `
        -RedirectStandardOutput (Join-Path $outputDirectory 'balanced.json') `
        -RedirectStandardError (Join-Path $outputDirectory 'balanced.stderr.txt')
    $captureExitCode = $process.ExitCode
    "capture-exited $([DateTimeOffset]::Now.ToString('O')) exit=$captureExitCode" |
        Set-Content -LiteralPath $stateFile -Encoding utf8
    @{ balanced = $captureExitCode } | ConvertTo-Json | Set-Content -LiteralPath (
